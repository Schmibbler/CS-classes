...
// Subtracting the trees recursively
node_t * subtractTree(node_t * t1, node_t * t2)
{
    // Simplest case of two existing nodes
    if (t1 != NULL && t2 != NULL) {
        t1->data = t1->data - t2->data;

        // Recursively setting the left and right children
        t1->leftchild = subtractTree(t1->leftchild, t2->leftchild);
        t1->rightchild = subtractTree(t1->rightchild, t2->rightchild);
    }
    // The case that the left tree node doesn't exist
    ...
    // The case that the right tree node doesn't exist
    else if (t1 != NULL && t2 == NULL) {
        return t1;
    }
    // The case that neither nodes exist
    else if (t1 == NULL && t2 == NULL) {
        return NULL;
    }
...
node_t * addTree(node_t * t1, node_t * t2)
{
    ...
    // The case that the left node doesn't exist
    else if (t1 == NULL && t2 != NULL) {
        // Because it's adding, simply create the new node
        // with the right node's data
        return createNode(t2->data);
    }
    // The case that the right node doesn't exist
    else if (t1 != NULL && t2 == NULL) {
        return t1;
    }
    // The case that neither nodes exist
